...
/**
 * Resilience-only copy of the content shape. The backend is the source of truth;
 * this is used solely so the marketing page still renders if the API is briefly
 * unreachable at render time (e.g. during a cold start). Keep in sync loosely —
 * the API value always wins when available.
 */
